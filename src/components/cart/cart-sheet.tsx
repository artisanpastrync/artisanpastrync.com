...
export function CartSheet({ cartItems, isOpen, onCartClose, isPending }: CartSheetProps) {
    const hasAnyItems = (cartItems?.length || 0) > 0;
    const subtotal =
        cartItems
            ?.map((item) => (item.variant.unit_amount ?? 0) * item.quantity)
            .reduce((acc, curr) => acc + curr, 0) ?? 0;

    const subtotalFormatted = formatStripeAmountForDisplay(subtotal);
    ...
    return (
        <Sheet open={isOpen} onOpenChange={() => onCartClose()}>
            <SheetContent className='size-full min-h-svh bg-white p-0'>
                ...
                <div
                    className={cn(
                        'mb-4 flex size-full h-[calc(100%-63px-260px)] flex-col gap-4 overflow-x-hidden p-4 pt-2'
                    )}
                >
